...
class Header extends React.Component<RouteComponentProps<{ query: string }>, HeaderState> {
    ...
    render(): React.ReactNode {
        return (
            <Sticky side={"top"}>
                <div data-tid={'header'} className={style.header}>
                    <div className={style.headerMenu}>
                        <div data-tid={'rubricMenuIcon'} ref={this.menu} onClick={this.handleRubricMenuClick} className={style.menu}>
                            <MenuIcon size={35} />
                        </div>
                        <Link data-tid='titleLink' to={"/"} className={style.title}>Библиотека</Link>
                        <IconButton icon={"DocumentTypeXml"} onClick={exportBooks} dataTid="xmlDownload"
                        />
