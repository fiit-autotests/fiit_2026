...
{
    [Test]
    public async Task SaveBook_ShouldThrowValidationException_IfRubricNotExist()
    {
        ...
        await action.Should().ThrowAsync<ValidationException>().WithMessage("Указана несуществующая рубрика.");
    }

    [Test]
    public async Task SaveBook_ShouldThrowValidationException_IfImageNotExist()
    {
        var book = GenerateBook(null);
        ...
        await action.Should().ThrowAsync<ValidationException>().WithMessage("Указана несуществующая картинка.");
    }
