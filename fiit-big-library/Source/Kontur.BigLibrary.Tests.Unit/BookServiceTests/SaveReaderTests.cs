...
{
    [Test]
    public async Task ShouldThrowValidationException_IfBookNotExist()
    {
        ...
        await action.Should().ThrowAsync<ValidationException>().WithMessage("Читать можно только существующие книги.");
    }

    [Test]
    public async Task ShouldThrowValidationException_IfBooksCountLessThanReadersCount()
    {
        SetDefaultMockSettings();
        var bookId = 123456;
    ...
    [Test]
    public async Task ShouldThrowValidationException_IfBooksReadersBookIdNotEqualToPassedBookId()
    {
        SetDefaultMockSettings();
        var bookId = 123456;
