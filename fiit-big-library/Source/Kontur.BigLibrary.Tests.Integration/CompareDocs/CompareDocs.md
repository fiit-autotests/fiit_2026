...
Cпособы проверок могут зависеть от уровня тестов.<br>
Например, на уровне интеграционных тестов мы проверяем все содержимое файла.<br>
...
<summary>1. Напишем тест на проверку непустого файла</summary>
   ...
   * Для создания книг использовать контейнер для bookService и imageService 
...
* При Smoke-тестировании, когда проверяется общая работоспособность системы;
* Если заранее неизвестны результаты выгрузки, например, при ИИ генерации данных;
...
<summary>4. Напишем тест на проверку количества выгруженных книг</summary>
   ...
   * Создадим в базе 5 книг, а лимит в фильтре укажем 4
...
**Не нужно всегда сравнивать весь файл целиком. Когда сценарий проверки предполагает изменение конкретного значения, лучше проверять только его. Это упростит чтение теста, поддержку, скорость прохождения, а также сделает тесты стабильнее**
...
<summary>6. Напишем approval test</summary>
   ...
   * Скопировать результат выгрузки и добавить его в файл Integration\Files\exportBooks.xml
